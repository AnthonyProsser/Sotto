...
All measured 2026-09-01 against controlled backdrops, for `WashView`. Relevant to any `.behindWindow` `NSVisualEffectView`; §4.2 still governs anything that should read as *glass*.

- **Shape it with `maskImage`, never an ancestor `layer.mask` and never `alphaValue < 1`.** Both of the latter force offscreen rendering, and an offscreen behind-window effect view silently stops sampling the backdrop — no error, no warning, a crisp backdrop and a flat tint. `maskImage` takes the same per-pixel alpha (an `NSImage` over a premultiplied-RGBA `CGImage` of black pixels at varying alpha) and keeps vibrancy alive. **This failure is invisible in a frame dump and invisible over a plain wallpaper** — check it over a striped or text backdrop, where a live blur is unmistakable.
- **There is no radius control, and no material has a smaller one.** Ten materials swept: they differ in tint *and* in transmission (`.hudWindow` passes 0.58 of the backdrop, `.sidebar`/`.toolTip`/`.underWindowBackground` 0.19), never in blur radius. Changing material cannot buy less blur. A weaker blur has to come from mixing untouched backdrop back in — bake a ceiling below 1 into the `maskImage` alpha.
- **Every material lifts pure black.** Over a black backdrop the field lands at 22–36 depending on material, lowest on `.hudWindow`. If a surface must stay black over black, that lift is the budget.
- **The window server blurs in gamma-encoded sRGB, not linear light.** Over a 50 % black/white backdrop the field measures 93 against the 94.5 an sRGB average predicts and the ~130 a linear one would. This is the real mechanism behind "blurring makes it look gray": bright detail averages toward a muddy mid-tone. **The app cannot correct it** — `CALayer.backgroundFilters` composites against the app's own layer tree, not the window server's pass, and is a no-op against a vibrancy backdrop. Owning the blur (ScreenCaptureKit, blur in linear space, redraw) is the only literal fix and it costs the Screen Recording grant, which is a product decision, not a rendering one.
- **Glass is not automatically the gentler option.** `NSGlassEffectView` lifts black *further* (27 `.clear`, 33 `.regular`) and dissolves backdrop text at least as completely. Measure before assuming §4.2's preference improves a given surface.
